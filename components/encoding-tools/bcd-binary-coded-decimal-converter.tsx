...
export default function BcdBinaryCodedDecimalConverter() {
  const [input, setInput] = useState<string>("")
  const [output, setOutput] = useState<string>("")
  const [mode, setMode] = useState<"decimal-to-bcd" | "bcd-to-decimal" | "bcd-to-binary">("decimal-to-bcd")
  ...
  const bcdToDecimal = useCallback((bcd: string, packed: boolean): string => {
    ...
    if (packed) {
      ...
      const digits = bytes.map(byte => {
        ...
        if (high > 9 || low > 9) throw new Error("Invalid BCD: digit > 9")
        return `${high}${low}`
      })
      ...
      const digits = bytes.map(byte => {
        ...
        if (value > 9) throw new Error("Invalid BCD: digit > 9")
  ...
  const bcdToBinary = useCallback((bcd: string, packed: boolean): string => {
    const decimal = bcdToDecimal(bcd, packed)
    const num = parseInt(decimal, 10)
    if (isNaN(num)) throw new Error("Invalid BCD value")
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold tracking-tight">BCD (Binary Coded Decimal) Converter</h2>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <Button
            variant={mode === "bcd-to-binary" ? "default" : "outline"}
            onClick={() => handleModeChange("bcd-to-binary")}
          >
            BCD to Binary
          </Button>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <Label htmlFor="input" className="text-base font-medium">
            {mode === "decimal-to-bcd" ? "Decimal Number" : "BCD Binary"}
          </Label>
        ...
        <Input
          ...
          placeholder={mode === "decimal-to-bcd" ? "Enter decimal number (e.g., 1234)..." : "Enter BCD binary (e.g., 00010010 00110100)..."}
        />
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <Label htmlFor="output" className="text-base font-medium">
            {mode === "decimal-to-bcd" ? "BCD Result" : mode === "bcd-to-decimal" ? "Decimal Result" : "Binary Result"}
          </Label>
          <Button
            variant="ghost"
            size="xs"
      ...
      <section className="rounded-lg border bg-muted/30 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Binary Coded Decimal (BCD) represents each decimal digit (0-9) using 4 bits (a nibble).
              It's commonly used in digital displays, calculators, and financial applications where
              exact decimal representation is important.
            </p>
            <p className="text-sm text-muted-foreground">
              Packed BCD stores two digits per byte (more efficient), while Unpacked BCD stores one
              digit per byte (simpler to process).
            </p>
          </div>
        </div>
      </section>
