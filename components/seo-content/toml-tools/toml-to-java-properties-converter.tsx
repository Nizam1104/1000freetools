...
export default function TomlToJavaPropertiesConverterSeo() {
  return (
    <div className="w-full max-w-4xl mx-auto space-y-8">
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold">How the TOML to Java Properties Converter Works</h2>
        <div className="space-y-3 text-muted-foreground">
          <p>
            This tool converts TOML configuration files into Java .properties file format. Paste your TOML content and get a flat key-value properties file compatible with Java's Properties class, Spring Boot, and other Java frameworks.
          </p>
          <p>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border bg-muted/30 p-4">
            <h3 className="font-medium mb-2">Migrating Spring Boot configs</h3>
            <p className="text-sm text-muted-foreground">
              Moving from TOML to application.properties? Convert your config format while maintaining the same settings. Spring Boot handles dot-notation automatically.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
            <h3 className="font-medium mb-2">Creating Java resource bundles</h3>
            <p className="text-sm text-muted-foreground">
              Store localized strings in TOML, convert to properties files for Java i18n. One TOML per language, generate messages_en.properties, messages_fr.properties, etc.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
            <h3 className="font-medium mb-2">Building legacy Java app configs</h3>
            <p className="text-sm text-muted-foreground">
              Older Java apps use .properties files. Define configs in modern TOML, convert to properties format. Easier editing, same runtime compatibility.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
            <h3 className="font-medium mb-2">Generating build configuration</h3>
            <p className="text-sm text-muted-foreground">
              Build tools like Gradle can use properties files. Define build settings in TOML, convert to gradle.properties. Better organization than raw properties.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
            <h3 className="font-medium mb-2">Creating JDBC connection configs</h3>
            <p className="text-sm text-muted-foreground">
              Database connection properties work well in .properties format. Define in TOML, convert for JDBC or connection pool configuration.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
        ...
        <div className="space-y-3 text-muted-foreground">
          ...
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
            <p className="text-sm text-amber-700 dark:text-amber-300">
              <strong>Pro tip:</strong> For Spring Boot, consider YAML instead of properties. Spring Boot supports YAML natively and preserves structure better than flat properties files.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border bg-muted/30 p-4">
            ...
            <p className="text-sm text-muted-foreground">
              This tool does TOML to properties. For properties to TOML, you'd need to parse dot-notation keys and reconstruct nested structure. Not a direct conversion.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
          ...
          <div className="rounded-lg border bg-muted/30 p-4">
            ...
            <p className="text-sm text-muted-foreground">
              Spring Boot supports comma-separated values: key=item1,item2,item3. Or use indexed keys: key[0]=a, key[1]=b. The converter uses indexed format.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
            <h3 className="font-medium mb-2">Can I use UTF-8 in properties files?</h3>
            <p className="text-sm text-muted-foreground">
              Traditional properties files use ISO-8859-1 with \u escapes. Java 6+ supports XML properties with UTF-8. Or use UTF-8 directly in modern Java versions.
            </p>
          </div>
          <div className="rounded-lg border bg-muted/30 p-4">
