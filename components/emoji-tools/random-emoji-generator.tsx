...
import { Copy, Check, Shuffle, Sparkles, Star, History, Filter, RotateCw } from "lucide-react"
...
const CATEGORIES = ["All", "Smileys", "Hearts", "Hands", "Animals", "Food", "Nature", "Symbols", "Activities", "Objects"]
...
export default function RandomEmojiGenerator() {
  ...
  const [gridEmojis, setGridEmojis] = useState<EmojiItem[]>([])
  const [copied, setCopied] = useState<string | null>(null)
  const [isSpinning, setIsSpinning] = useState(false)

  const generateRandom = useCallback(() => {
    const filtered = selectedCategory === "All" ? EMOJI_DATA : EMOJI_DATA.filter((e) => e.category === selectedCategory)
  ...
  const generateGrid = useCallback(() => {
    ...
    setGridEmojis(results)
  }, [selectedCategory, gridSize])
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6">
      <Tabs defaultValue="single" className="w-full">
        <TabsList className="w-full justify-start overflow-x-auto">
          <TabsTrigger value="single" className="text-sm px-4 py-2"><Sparkles className="size-4 mr-2" />Single</TabsTrigger>
          <TabsTrigger value="grid" className="text-sm px-4 py-2"><Star className="size-4 mr-2" />Grid</TabsTrigger>
          <TabsTrigger value="wheel" className="text-sm px-4 py-2"><Shuffle className="size-4 mr-2" />Wheel</TabsTrigger>
        </TabsList>
              ...
              <div className="rounded-lg border bg-muted/30 p-8 flex flex-col items-center justify-center">
                ...
                <div className="flex flex-wrap gap-1 mt-2 justify-center">
                  {randomEmoji.keywords.map((keyword) => (
                    <span key={keyword} className="text-xs px-2 py-0.5 bg-muted rounded-full">{keyword}</span>
                  ))}
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={() => copyToClipboard(randomEmoji.emoji, "emoji")}>
                  {copied === "emoji" ? <Check className="size-4 mr-1" /> : <Copy className="size-4 mr-1" />}
                  Copy Emoji
                </Button>
                <Button variant="outline" size="sm" onClick={() => copyToClipboard(`${randomEmoji.emoji} ${randomEmoji.name}`, "name")}>
          ...
          {history.length > 0 && (
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-base font-medium flex items-center gap-2"><History className="size-4" />Recent Emojis</Label>
                <Button variant="ghost" size="sm" onClick={clearHistory}>Clear</Button>
              </div>
              <div className="flex flex-wrap gap-2">
                {history.map((item, idx) => (
                  <button key={idx} onClick={() => copyToClipboard(item.emoji, `history-${idx}`)} className="text-3xl p-3 rounded-lg bg-muted hover:bg-muted/80 transition-colors" title={item.name}>
                    {item.emoji}
                  </button>
                ))}
              </div>
        ...
        <TabsContent value="grid" className="space-y-6 mt-4">
          {/* Grid Size */}
          <section className="space-y-3">
            <Label className="text-base font-medium">Grid Size: {gridSize[0]} emojis</Label>
            <Slider value={gridSize} onValueChange={setGridSize} min={4} max={25} step={1} className="w-full" />
          </section>

          {/* Generate Grid */}
          <Button onClick={generateGrid} className="w-full sm:w-auto">
            <Shuffle className="size-4 mr-2" />
            Generate Grid
          </Button>
          ...
          {gridEmojis.length > 0 && (
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-base font-medium">Random Grid</Label>
                <Button variant="outline" size="sm" onClick={() => copyToClipboard(gridEmojis.map((e) => e.emoji).join(" "), "grid")}>
                  {copied === "grid" ? <Check className="size-4 mr-1" /> : <Copy className="size-4 mr-1" />}
                  Copy All
                </Button>
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
                {gridEmojis.map((item, idx) => (
                  <button key={idx} onClick={() => copyToClipboard(item.emoji, `grid-${idx}`)} className="rounded-lg border p-4 text-center hover:bg-muted/50 transition-all">
        ...
        <TabsContent value="wheel" className="space-y-6 mt-4">
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-center">
              <div className={cn("w-48 h-48 rounded-full border-4 border-primary flex items-center justify-center transition-transform", isSpinning && "animate-spin")}>
            ...
            <Button onClick={spinWheel} disabled={isSpinning} className="w-full sm:w-auto">
              <RotateCw className={cn("size-4 mr-2", isSpinning && "animate-spin")} />
              {isSpinning ? "Spinning..." : "Spin the Wheel"}
            </Button>
          </section>

          {/* Result */}
          {randomEmoji && !isSpinning && (
            <section className="space-y-3">
              <Label className="text-base font-medium">Wheel Result</Label>
              <div className="rounded-lg border bg-muted/30 p-6 flex flex-col items-center justify-center">
                <span className="text-8xl mb-4">{randomEmoji.emoji}</span>
                <h3 className="text-xl font-semibold">{randomEmoji.name}</h3>
                <p className="text-sm text-muted-foreground">{randomEmoji.category}</p>
              </div>
              <Button variant="outline" size="sm" onClick={() => copyToClipboard(randomEmoji.emoji, "wheel")}>
                {copied === "wheel" ? <Check className="size-4 mr-1" /> : <Copy className="size-4 mr-1" />}
                Copy Emoji
              </Button>
            </section>
          )}
