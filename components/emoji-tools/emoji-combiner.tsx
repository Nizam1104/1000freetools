...
const POPULAR_COMBOS: EmojiCombo[] = [
  ...
  { id: "11", emojis: ["💰", "📈"], name: "Growing Wealth", description: "Financial success" },
  { id: "12", emojis: ["🏆", "🥇"], name: "Champion", description: "Victory achieved" },
]
...
export default function EmojiCombiner() {
  ...
  const combineEmojis = useCallback(() => {
    ...
    // Create a combined emoji string with special formatting
  ...
  const downloadAsPNG = useCallback(() => {
    ...
    if (!canvas || selectedEmojis.length === 0) return
    ...
    // Draw emojis
    const emojiSize = size / Math.max(selectedEmojis.length, 1)
    const baseSize = 80 * scale[0]

    selectedEmojis.forEach((emoji, index) => {
      const x = (size / 2) + Math.cos((index * Math.PI * 2) / selectedEmojis.length) * 50 * (rotation[0] / 360)
      const y = (size / 2) + Math.sin((index * Math.PI * 2) / selectedEmojis.length) * 50 * (rotation[0] / 360)
    ...
    link.click()
  }, [selectedEmojis, opacity, scale, rotation, backgroundColor])
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-10 gap-2 p-4 rounded-lg border bg-muted/30">
          {EMOJI_PALETTE.map((emoji) => (
            <button
              ...
              className="aspect-square flex items-center justify-center text-2xl hover:bg-muted rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-ring"
              title={`Add ${emoji}`}
            >
              {emoji}
            </button>
        ...
        <section className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 p-4 rounded-lg border bg-background">
            {selectedEmojis.map((emoji, index) => (
              <div
                key={index}
                className="relative group flex items-center justify-center w-12 h-12 rounded-lg bg-muted hover:bg-destructive/20 transition-colors"
              >
                <span className="text-2xl">{emoji}</span>
                <button
                  onClick={() => removeEmoji(index)}
                  className="absolute -top-1 -right-1 size-4 rounded-full bg-destructive text-destructive-foreground opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-xs"
                >
                  ×
                </button>
              </div>
      ...
      <section className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={combineEmojis} disabled={selectedEmojis.length === 0}>
        ...
        <section className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => copyToClipboard(combinedResult, "result")}
            >
