...
export default function BulkEmoji() {
  ...
  const getSelectedEmojis = useCallback(() => {
    ...
    // Apply skin tone modifier to applicable emojis
    const skinTone = SKIN_TONES.find((t) => t.id === selectedSkinTone)?.modifier || ""
    if (skinTone) {
      // Apply to hand emojis and people
  ...
  const downloadPack = useCallback(async () => {
    ...
    if (outputFormat === "text") {
      ...
      // For PNG/SVG, simulate download (in real app, would generate images)
      setTimeout(() => {
        alert(`Would download ${emojis.length} emojis as ${outputFormat.toUpperCase()} at ${selectedSize}x${selectedSize}px\n\nIn a production environment, this would generate individual image files packaged in a ZIP archive.`)
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    from {selectedPacks.length} pack{selectedPacks.length !== 1 ? "s" : ""}
                  </p>
                </div>
                <Button onClick={() => {}}>
                  Continue to Export
        ...
        <TabsContent value="preview" className="space-y-6 mt-4">
          {totalEmojis === 0 ? (
            ...
            <>
              ...
              <section className="space-y-3">
                ...
                <div className="flex flex-wrap gap-4 p-4 rounded-lg border bg-muted/30">
                  {["👍", "👋", "🙏", "👏", "✊"].map((emoji, idx) => {
                    ...
                    const modifiedEmoji = emoji + modifier
                    return (
                      <div key={idx} className="text-center">
                        <span className="text-4xl">{modifiedEmoji}</span>
                      </div>
                    )
                  })}
                </div>
              ...
              <section className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => copyToClipboard(selectedEmojis.join(""), "all")}
                  >
              ...
              {/* By Category */}
              <section className="space-y-3">
                <Label className="text-base font-medium">By Category</Label>
                <div className="space-y-4">
                  {EMOJI_PACKS.filter((p) => selectedPacks.includes(p.id)).map((pack) => (
                    <div key={pack.id} className="rounded-lg border p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium">{pack.name}</h4>
                        <span className="text-sm text-muted-foreground">{pack.emojis.length} emojis</span>
                      </div>
                      <div className="flex flex-wrap gap-1">
            ...
            <>
              ...
              <section className="rounded-lg border bg-muted/30 p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  <div>
                    <p className="text-muted-foreground">Total Emojis</p>
                    <p className="text-2xl font-bold">{totalEmojis}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Packs</p>
                    <p className="text-2xl font-bold">{selectedPacks.length}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Size</p>
                    <p className="text-2xl font-bold">{selectedSize}px</p>
                  </div>
                  <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  variant="outline"
                  onClick={() => copyToClipboard(JSON.stringify(selectedEmojis), "json")}
                >
