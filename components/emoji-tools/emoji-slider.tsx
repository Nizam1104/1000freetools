...
export default function EmojiSlider() {
  ...
  const downloadAsImage = useCallback(() => {
    ...
    const text = generatedScale.join(" ")
    const blob = new Blob([text], { type: "text/plain" })
    const url = URL.createObjectURL(blob)
    const link = document.createElement("a")
    link.download = "emoji-scale.txt"
    link.href = url
    link.click()
  ...
  const getEmbedCode = useCallback(() => {
    const emojis = generatedScale.join("")
    return `<div class="emoji-scale">${emojis}</div>`
  }, [generatedScale])

  const incrementSteps = () => {
    setSteps([Math.min(20, steps[0] + 1)])
  }

  const decrementSteps = () => {
    setSteps([Math.max(2, steps[0] - 1)])
  }
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input
              id="start-emoji"
              value={startEmoji}
              onChange={(e) => setStartEmoji(e.target.value[0] || "😢")}
              maxLength={2}
          ...
          <div className="space-y-2">
            ...
            <Input
              id="end-emoji"
              value={endEmoji}
              onChange={(e) => setEndEmoji(e.target.value[0] || "😍")}
              maxLength={2}
              className="text-2xl text-center"
            />
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={decrementSteps} disabled={steps[0] <= 2}>
            ...
            <Slider
              ...
              className="flex-1"
            />
            <Button variant="outline" size="icon" onClick={incrementSteps} disabled={steps[0] >= 20}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input
              id="start-label"
              value={labelText.start}
              onChange={(e) => setLabelText({ ...labelText, start: e.target.value })}
              placeholder="e.g., Bad"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              id="end-label"
              value={labelText.end}
              onChange={(e) => setLabelText({ ...labelText, end: e.target.value })}
        ...
        <section className="space-y-3">
          ...
          >
            {orientation === "horizontal" ? (
              ...
              <div className="flex items-center gap-2">
                {showLabels && (
                  <span className="text-sm text-muted-foreground writing-mode-vertical">{labelText.start}</span>
                )}
                ...
                {showLabels && (
                  <span className="text-sm text-muted-foreground writing-mode-vertical">{labelText.end}</span>
                )}
              </div>
            )}
          </div>
        ...
        <section className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => copyToClipboard(generatedScale.join(" "), "scale")}
            >
