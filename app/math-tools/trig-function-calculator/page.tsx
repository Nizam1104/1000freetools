...
export default function TrigFunctionCalculator() {
  ...
  const [selectedFunction, setSelectedFunction] = useState<"all" | "sin" | "cos" | "tan" | "csc" | "sec" | "cot">("all");
  ...
  const calculate = () => {
    ...
    const angleInDeg = unit === "degrees" ? angleNum : (angleNum * 180 / Math.PI);
    ...
    const getExactValue = (func: string, ang: number): string => {
      const normalizedDeg = unit === "degrees" ? ang % 360 : (ang * 180 / Math.PI) % 360;
    ...
    const results: Record<string, any> = {
      ...
      csc: { name: "Cosecant", value: isUndefined(csc) ? undefined : csc, symbol: "csc" },
      sec: { name: "Secant", value: isUndefined(sec) ? undefined : sec, symbol: "sec" },
      cot: { name: "Cotangent", value: isUndefined(cot) ? undefined : cot, symbol: "cot" }
    };

    const normalizedAngle = normalizeAngle(angleNum);
    
    const quadrant = unit === "degrees" 
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8">
      <div className="mb-8">
        <h1 className="text-3xl font-semibold mb-2">Trig Function Calculator – Calculate Sin Cos Tan Online</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter an angle and calculate sine, cosine, tangent, and their reciprocals.
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="space-y-6">
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="p-4 bg-muted rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">Angle (degrees)</p>
                    <p className="text-2xl font-bold">{result.angleInDeg}°</p>
                  </div>
                  <div className="p-4 bg-muted rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">Angle (radians)</p>
                    <p className="text-2xl font-bold">{result.angleInRad} rad</p>
                  </div>
                  <div className="p-4 bg-muted rounded-lg">
                    <p className="text-sm text-muted-foreground mb-1">Quadrant</p>
                    <p className="text-2xl font-bold">Q{result.quadrant}</p>
                    <p className="text-xs text-muted-foreground mt-1">{result.quadrantInfo}</p>
                  </div>
                </div>

                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.entries(result.results).map(([key, data]: [string, any]) => {
                    if (selectedFunction !== "all" && key !== selectedFunction) return null;
                    ...
                    return (
                      <div key={key} className="p-4 bg-muted rounded-lg">
                        ...
                        <p className="text-lg font-mono mb-1">
                          {data.symbol}({result.angle}°) = {isUndefined ? "undefined" : data.value.toFixed(6)}
                        </p>
                        {!isUndefined && (
                          <p className="text-xs text-muted-foreground">
                            Exact: {result.getExactValue(key, result.angle) || "N/A"}
                          </p>
                        )}
                      </div>
                    );
                ...
                <div className="p-4 bg-muted rounded-lg">
                  <h4 className="font-semibold text-sm mb-3">Unit Circle Reference</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="font-semibold mb-2">Common Angles (Degrees)</p>
                    ...
                    <div>
                      <p className="font-semibold mb-2">Common Angles (Radians)</p>
                      <div className="space-y-1 text-xs text-muted-foreground">
            ...
            <div className="p-4 border rounded-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-muted-foreground">opposite / hypotenuse</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-mono">cos(θ)</span>
                  <span className="text-xs text-muted-foreground">adjacent / hypotenuse</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-mono">tan(θ)</span>
                  <span className="text-xs text-muted-foreground">opposite / adjacent</span>
                </div>
              </div>
            </div>

            <div className="p-4 border rounded-lg">
              <h4 className="font-semibold text-sm mb-2">Reciprocal Functions</h4>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-mono">csc(θ)</span>
                  <span className="text-xs text-muted-foreground">1 / sin(θ)</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-mono">sec(θ)</span>
                  <span className="text-xs text-muted-foreground">1 / cos(θ)</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-mono">cot(θ)</span>
                  <span className="text-xs text-muted-foreground">1 / tan(θ)</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 border rounded-lg">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-muted-foreground">Domain: all reals, Range: [-1, 1]</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono">tan</span>
                  <span className="text-muted-foreground">Domain: θ ≠ 90° + 180°n, Range: all reals</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono">csc</span>
                  <span className="text-muted-foreground">Domain: θ ≠ 180°n, Range: (-∞,-1] ∪ [1,∞)</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono">sec</span>
                  <span className="text-muted-foreground">Domain: θ ≠ 90° + 180°n, Range: (-∞,-1] ∪ [1,∞)</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-mono">cot</span>
                  <span className="text-muted-foreground">Domain: θ ≠ 180°n, Range: all reals</span>
                </div>
              </div>
            </div>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  { deg: 0, rad: "0", sin: "0", cos: "1", tan: "0", csc: "—", sec: "1", cot: "—" },
                  { deg: 30, rad: "π/6", sin: "1/2", cos: "√3/2", tan: "√3/3", csc: "2", sec: "2√3/3", cot: "√3" },
                  { deg: 45, rad: "π/4", sin: "√2/2", cos: "√2/2", tan: "1", csc: "√2", sec: "√2", cot: "1" },
                  { deg: 60, rad: "π/3", sin: "√3/2", cos: "1/2", tan: "√3", csc: "2√3/3", sec: "2", cot: "√3/3" },
                  { deg: 90, rad: "π/2", sin: "1", cos: "0", tan: "—", csc: "1", sec: "—", cot: "0" },
                  { deg: 180, rad: "π", sin: "0", cos: "-1", tan: "0", csc: "—", sec: "-1", cot: "—" },
                  { deg: 270, rad: "3π/2", sin: "-1", cos: "0", tan: "—", csc: "-1", sec: "—", cot: "0" },
                  { deg: 360, rad: "2π", sin: "0", cos: "1", tan: "0", csc: "—", sec: "1", cot: "—" }
                ].map((row) => (
                  <tr key={row.deg} className="border-b">
                    <td className="p-2 font-mono">{row.deg}° ({row.rad})</td>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div>
            <h4 className="font-semibold text-sm mb-2">When is tangent undefined?</h4>
            <p className="text-xs text-muted-foreground">
          ...
          <div>
            <h4 className="font-semibold text-sm mb-2">Why are some cosecant and secant values undefined?</h4>
            <p className="text-xs text-muted-foreground">
          ...
          <div>
            <h4 className="font-semibold text-sm mb-2">How do I convert between degrees and radians?</h4>
            <p className="text-xs text-muted-foreground">
          ...
          <div>
            <h4 className="font-semibold text-sm mb-2">What does it mean that trig functions are periodic?</h4>
            <p className="text-xs text-muted-foreground">
          ...
          <div>
            <h4 className="font-semibold text-sm mb-2">How do I know which quadrant an angle is in?</h4>
            <p className="text-xs text-muted-foreground">
          ...
          <div>
            <h4 className="font-semibold text-sm mb-2">What are exact values vs decimal approximations?</h4>
            <p className="text-xs text-muted-foreground">
