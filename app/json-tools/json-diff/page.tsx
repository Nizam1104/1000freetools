...
export default function JsonDiffPage() {
  ...
  const computeDiff = useCallback((obj1: any, obj2: any, path: string = ""): DiffResult[] => {
    ...
    if (Array.isArray(obj1) && Array.isArray(obj2)) {
      ...
      for (let i = 0; i < maxLength; i++) {
        ...
        if (i >= obj1.length) {
          ...
          if (childDiffs.length > 0) {
            results.push({ key, type: "changed", oldValue: obj1[i], newValue: obj2[i], children: childDiffs });
          }
        }
      }
      ...
      for (const key of allKeys) {
        ...
        if (!(key in obj1)) {
          ...
          if (childDiffs.length > 0) {
            results.push({ key, type: "changed", oldValue: obj1[key], newValue: obj2[key], children: childDiffs });
          }
        }
      }
    } else if (obj1 !== obj2) {
  ...
  const compareJson = useCallback(() => {
    ...
    toast.success(`Diff complete: ${added} added, ${removed} removed, ${changed} changed`);
  ...
  const loadSample = () => {
    setJson1(JSON.stringify({ name: "John", age: 30, city: "NYC", hobbies: ["reading"] }, null, 2));
    setJson2(JSON.stringify({ name: "John", age: 31, country: "USA", hobbies: ["reading", "coding"] }, null, 2));
  };
  ...
  const stats = useMemo(() => {
    ...
    const countByType = (type: DiffType) => diffResult.filter(d => d.type === type).length;
    return {
      added: countByType("added"),
      removed: countByType("removed"),
      changed: countByType("changed"),
      unchanged: diffResult.length - countByType("added") - countByType("removed") - countByType("changed"),
    };
  }, [diffResult]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-semibold tracking-tight mb-2">JSON Diff Tool – Compare Two JSON Objects</h1>
            ...
            <CardContent className="p-4">
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">Added: <span className="font-semibold">{stats.added}</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span className="text-sm">Removed: <span className="font-semibold">{stats.removed}</span></span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-yellow-500" />
                  <span className="text-sm">Changed: <span className="font-semibold">{stats.changed}</span></span>
                </div>
              </div>
            </CardContent>
          </Card>
