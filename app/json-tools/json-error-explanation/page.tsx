    ...
    try {
      ...
      if (lowerMsg.includes("unexpected token")) {
        if (lowerMsg.includes("string")) {
          explanation = "The parser found an unexpected string or character. This often happens when there's a missing comma, colon, or quote.";
          fix = "Check the character before the error position. Make sure strings are wrapped in double quotes and properties are separated by commas.";
        } else if (lowerMsg.includes("number")) {
          explanation = "There's a number format issue. This could be an invalid number like '1.2.3' or a number starting with a decimal point.";
          fix = "Ensure numbers are in valid format (e.g., 123, -45.67, 0.5). Numbers cannot start with a decimal point.";
        } else if (lowerMsg.includes("identifier")) {
          explanation = "An unquoted property name was found. In JSON, all property names must be strings wrapped in double quotes.";
          fix = "Wrap the property name in double quotes. Change {name: \"value\"} to {\"name\": \"value\"}.";
        } else {
          explanation = "The parser encountered something it didn't expect at this position in the JSON structure.";
          fix = "Check for: missing commas between items, extra commas at the end of arrays/objects, mismatched brackets, or unquoted strings.";
        }
      } else if (lowerMsg.includes("string")) {
        explanation = "There's a string syntax error. This usually means a string is not properly closed or contains invalid escape sequences.";
        fix = "Make sure all strings use double quotes (\") not single quotes ('). Check for unescaped quotes or backslashes inside strings.";
      } else if (lowerMsg.includes("number")) {
        explanation = "The number format is invalid. JSON has strict rules about number formatting.";
        fix = "Numbers cannot have leading zeros (except 0 itself), cannot end with a decimal point, and cannot use Infinity or NaN.";
      } else if (lowerMsg.includes("property") || lowerMsg.includes("key")) {
        explanation = "There's an issue with an object property. Property names in JSON must be strings.";
        fix = "Ensure all property names are wrapped in double quotes. Example: {\"name\": \"value\"} not {name: \"value\"}.";
      } else if (lowerMsg.includes("bracket") || lowerMsg.includes("brace")) {
        explanation = "There's a mismatch in brackets or braces. Every opening bracket must have a matching closing bracket.";
        fix = "Count your opening and closing brackets. Make sure [ matches ], { matches }, and they're properly nested.";
      } else {
        explanation = `Parser error: ${errorMsg}`;
        fix = "Review the JSON syntax carefully. Common issues include missing commas, unquoted strings, or mismatched brackets.";
      }

      // Find the problematic character
      const context = input.substring(Math.max(0, position - 20), Math.min(input.length, position + 20));
      if (context) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-semibold tracking-tight mb-2">JSON Error Explainer – Fix JSON Errors Online</h1>
          ...
          <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="text-muted-foreground">Error:</span> <code className="bg-muted px-2 py-1 rounded">{errorInfo.error}</code></p>
                      <p><span className="text-muted-foreground">Position:</span> Line {errorInfo.position.line}, Column {errorInfo.position.column}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <p className="text-muted-foreground whitespace-pre-wrap">{errorInfo.fix}</p>
              </CardContent>
            </Card>
          </div>
        )}
